...
export class StrataWeb extends WebPlugin implements StrataPlugin, StrataPlatformAdapter {
    ...
    /**
     * @deprecated Use triggerHaptics() with duration option instead.
     * Note: This method only uses navigator.vibrate (no gamepad haptics) to maintain backward compatibility.
     */
    async vibrate(options?: { duration?: number }): Promise<void> {
        console.warn(
            'vibrate() is deprecated. Use triggerHaptics({ intensity: "medium", duration: ... }) instead.'
        );
        // Maintain backward compatibility: only use navigator.vibrate, no gamepad haptics
        if ('vibrate' in navigator) {
    ...
    async triggerHaptic(options: TriggerHapticOptions): Promise<void> {
        console.warn(
            'triggerHaptic() is deprecated. Use triggerHaptics() with customIntensity and duration options instead.'
        );
        const pattern = options.pattern;
        if (!pattern) {
            await this.triggerHaptics({ intensity: 'light', duration: 50 });
            return;
        }

        // Convert old pattern format to new format
        // Map numeric intensity (0-1) to customIntensity
        await this.triggerHaptics({
            intensity: 'medium',
            customIntensity: pattern.intensity,
